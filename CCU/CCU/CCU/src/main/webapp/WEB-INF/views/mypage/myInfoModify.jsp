		...
		$("#certification").click(function(){
			...
			popup : false // PC환경에서는 popup 파라메터가 무시되고 항상 true 로 적용됨
				...
				if (rsp.success) { // 인증 성공 시
					//alert("인증성공"+rsp.imp_uid);
					// jQuery로 HTTP 요청
					$.ajax({
						url: "/mypage/certificationOk", // 예: https://www.myservice.com/certifications
						method: "POST",
						dataType: "json",
...
<div class="container">
	...
	<table id="content">
		...
		<tr class="rows">
			<th>본인인증</th>
			<td>
				<c:if test="${vo.self_certification eq 0}"><c:out value="N"/>
				<input type="button" value="본인인증하기" id="certification"/>
				</c:if>
